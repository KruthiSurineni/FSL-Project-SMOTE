...
def smoteforMamography(T, N, k, minoritySamples, fo):
    if (N < 100):
        T = (N / 100) * T
        N = 100
    ...
    print ("Mamography Smoted - Synthetic samples added: " + str(N * T))
    ...
    while (N != 0):
        ...
        for attr in range(1, numattrs):
            ...
            for i in range(0, 6):
                ...
                Synthetic.append(round(float(minoritySamples[currentIndex][i]) + (gap * dif), 8))
...
def printToFile(Synthetic, fo):
    outputString = (
        str(Synthetic[0]) + "," + str(Synthetic[1]) + "," + str(Synthetic[2]) + "," + str(
            Synthetic[3]) + "," + str(Synthetic[4]) + "," + str(Synthetic[5]) + "," + str(Synthetic[6]))
